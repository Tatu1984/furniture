...
const menusData: Record<string, MenuData> = {
  "1": {
    id: "1",
...
export default function MenuEditorPage({
  ...
  const initial = menusData[id] ?? menusData["1"];

  const [menuName, setMenuName] = React.useState(initial.name);
  const [menuSlug, setMenuSlug] = React.useState(initial.slug);
  const [menuLocation, setMenuLocation] = React.useState<MenuLocation>(initial.location);
  const [menuStatus, setMenuStatus] = React.useState(initial.status);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={menuName} description="Edit menu structure and items">
          ...
          {editingItem && (
            <div className="space-y-4 py-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="edit-new-tab" className="text-sm font-normal">
                  Open in new tab
                </Label>
              </div>
            </div>
